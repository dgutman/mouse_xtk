...
function init_viewer2d() {
	...
	sliceX.interactor.onMouseMove = function() {
		if (_MOUSEBRAINID_.hover){
			clearTimeout(_MOUSEBRAINID_.hover);
		}
		_MOUSEBRAINID_.hover = setTimeout(on2DHover.bind(this, sliceX), 100);
	};
	...
	sliceY.interactor.onMouseMove = function() {
		...
		_MOUSEBRAINID_.hover = setTimeout(on2DHover.bind(this, sliceY), 100);
	};
	...
	sliceZ.interactor.onMouseMove = function() {
		...
		_MOUSEBRAINID_.hover = setTimeout(on2DHover.bind(this, sliceZ), 100);
	  };
    ...
    $( "#sliceX_val" ).val( $("#yellow_slider").slider("value"));
	$( "#sliceY_val" ).val( $("#red_slider").slider("value"));
	$( "#sliceZ_val" ).val( $("#green_slider").slider("value"));
	
	
	 $( "#slices_in_X" ).val( sliceX.Xb.length);
	 $( "#slices_in_Y" ).val( sliceY.Xb.length);
	 $( "#slices_in_Z" ).val( sliceZ.Xb.length);
...
function on2DHover(renderer) {
	...
	if (!_MOUSEBRAINID_.hoverLabelSelect){
		return;
	}
