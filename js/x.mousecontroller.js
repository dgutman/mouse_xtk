...
function toggleVolumeRendering() {
	var volume = _MOUSEBRAINID_.volumes[_MOUSEBRAINID_.currentVolume];
...
function toggleLabelmapVisibility(label) {
	...
	// remember opacity
	_MOUSEBRAINID_.labelOpacity = volume.labelmap.opacity;
...
function toggleMeshVisibility(label) {
	...
	if (!_MOUSEBRAINID_.meshes[_MOUSEBRAINID_.currentVolume][label]) {
		// load mesh
		var m = new X.mesh();
		m.file = "data/"+_MOUSEBRAINID_.steps[_MOUSEBRAINID_.currentVolume]+"/"+label;
		m.caption = label.replace(".vtk","").split("_")[2].split();//.replace("Model_","")
		console.log(label);
		r0.add(m);
		// grab label value
		var labelvalue = label.replace("Model_","").split("_")[0];
		m.color = _MOUSEBRAINID_.volumes[0].labelmap.colortable.get(labelvalue).slice(0).splice(1,3);
		_MOUSEBRAINID_.meshes[_MOUSEBRAINID_.currentVolume][label] = m;
		_MOUSEBRAINID_.meshes[_MOUSEBRAINID_.currentVolume][label].visible = false;
		_MOUSEBRAINID_.meshes[_MOUSEBRAINID_.currentVolume][label].opacity = _MOUSEBRAINID_.meshOpacity;
	}
	// show the mesh
	_MOUSEBRAINID_.meshes[_MOUSEBRAINID_.currentVolume][label].visible = !_MOUSEBRAINID_.meshes[_MOUSEBRAINID_.currentVolume][label].visible;
}
...
function opacityMesh(event, ui) {
	...
	for (var m in _MOUSEBRAINID_.meshes[_MOUSEBRAINID_.currentVolume]) {
		if (_MOUSEBRAINID_.meshes[_MOUSEBRAINID_.currentVolume][m] != null) {
			_MOUSEBRAINID_.meshes[_MOUSEBRAINID_.currentVolume][m].opacity = ui.value / 100;		
		}
	}
	
	_MOUSEBRAINID_.meshOpacity = ui.value / 100;

	//_MOUSEBRAINID_.meshes[_MOUSEBRAINID_.currentVolume]['Model_3_Left-Cerebral-Cortex.vtk'].opacity = ui.value / 100;
/*  if (RT.linked) {
